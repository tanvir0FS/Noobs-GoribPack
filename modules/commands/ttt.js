...
function move(x, y, data) {
  ...
  } else return "[🤍] এই বক্স টি ইতিমধ্যে ব্লক করা হয়েছে, \n[🤍] অন্য বক্স সিলেক্ট করুন।";
...
module.exports.handleReply = async function({ event, api, handleReply }) {
  ...
  if(!isNaN(number) && number > 0 && number < 10) {
    ...
    if(checkGameOver(data)) {
      var gayban = ["তুমি নুব😆 তোমাকে হারিয়ে দিয়েছি😆", "তুমি এখনো নুব, প্রো হয়ে খেলতে আসবা😂", "তুমি নুব, পারবে না আমার সাথে😆", "কখনো পারবা না আমার সাথে🥱", "তুমি looser, আমি জিতে গেছি😘", "তোমাকে হারানো টা একদম সহজ ছিলো🤣"];
      if(checkAIWon(data)) lmao = `You lose! ${gayban[Math.floor(Math.random() * gayban.length)]}`;
      else if(checkPlayerWon(data)) lmao = "ব্রিলিয়ান্ট 🙂, তুমি Ai কে হারিয়ে দিয়েছো \n Congratulations 🌸";
      else lmao = "ড্র করতে পারবা, আমাকে কখনো হারাতে পারবা না পিও😂";
      global.moduleData.tictactoe.delete(threadID);
    }
    var msg = lmao !== "" ? lmao : temp == undefined ? "[🤍] বক্স নাম্বার রিপ্লে করুন:" : temp;
  ...
  } else return api.sendMessage("[❗] ১ থেকে ৯  এর মধ্যে সিলেক্ট করুন [english]", threadID, messageID);
}
...
module.exports.run = async function ({ event, api, args }) {
  ...
  let newData;
  if (args.length == 0) return api.sendMessage("[🤍] অনুগ্রহ করে X অথবা O সিলেক্ট করুন। \n[🤍] example: .ttt X", threadID, messageID);
  if (args[0].toLowerCase() == "delete") {
    global.moduleData.tictactoe.delete(threadID);
    return api.sendMessage("Removed chessboard!", threadID, messageID);
  }
  if (args[0].toLowerCase() == "continue") {
    if (!data.gameOn) return api.sendMessage("No data! use " + concak + "X অথবা O দিয়ে আবার খেলুন।", threadID, messageID);
    return api.sendMessage({ body: "বক্স ব্লক করতে নাম্বার রিপ্লে করো।", attachment: await displayBoard(data)}, threadID, (error, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        author: senderID,
        messageID: info.messageID
  ...
  if (!data.gameOn) {
    ...
    if (abc !== "x" && abc !== "o") return api.sendMessage("[🤍] অনুগ্রহ করে X অথবা O সিলেক্ট করুন। \n[🤍] example: .ttt X", threadID, messageID);
    if (abc == "o") {
      newData = startBoard({ isX: false, data, threadID });
      api.sendMessage({ body: "তুমি শুরু করো বক্স নাম্বার রিপ্লে করলে বক্স ব্লক হবে", attachment: await displayBoard(newData)}, threadID, (error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
    ...
    if (abc == "x") {
      ...
      AIStart(newData);
      api.sendMessage({ body: "আমি আগে শুরু করেছি,\nবক্স নাম্বার রিপ্লে করলে বক্স ব্লক হবে।", attachment: await displayBoard(data)}, threadID,(error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
