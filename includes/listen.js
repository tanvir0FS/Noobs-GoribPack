module.exports = function({ api, models }) {
  ...
  const checkttDataPath = __dirname + '/../Cmds-Events/commands/checktt/';
  setInterval(async() => {
...
(async function () {
	...
	logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Bot was made by Nazrul" : global.config.BOTNAME}`, "[ BOT INFO ]");
	...
	//DEFINE DATLICH PATH
	const datlichPath = __dirname + '/../Cmds-Events/Commands/cache/datlich.json';
		...
		for (el of temp) {
			...
			if ("ATTACHMENT" in el) {
				...
				for (a of el.ATTACHMENT) {
					...
					fs.writeFileSync(__dirname + `/../Cmds-Events/commands/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
					out.attachment.push(fs.createReadStream(__dirname + `/../Cmds-Events/commands/cache/${a.fileName}`));
				}
			}
			console.log(out);
			if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
			api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `/../Cmds-Events/commands/cache/${a.fileName}`)) : "");
		}
